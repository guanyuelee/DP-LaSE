...
def main(args):
    ...
    if args.optim == 'Adam':
        optimizer = optim.Adam(params=D0_list, lr=args.lr)
    elif args.optim == 'SGD':
        optimizer = optim.SGD(params=D0_list, lr=args.lr)
    else:
    ...
    for iter in range(args.total_iterations):
        ...
        d_list = [torch.matmul(which_d, D_item).view(batch_size, dim_z, 1, 1) for D_item in D_list]
        ...
        d2_list = [torch.matmul(which_d2, D_item).view(batch_size, dim_z, 1, 1) for D_item in D_list]
        ...
        W_t2s = []
        for i in range(len(layer_list)):
            z_t = density_preserving_traverse(z, d_list[i], t)
            z_t2 = density_preserving_traverse(z, d2_list[i], t)
            # we only select the needed layer.
            w = generator([z], which_block=8, pre_model=True)
            w_t = generator([z_t], which_block=8, pre_model=True)  # shape is [bs, 19, 512]
            w_t2 = generator([z_t2], which_block=8, pre_model=True)  # shape is [bs, 19, 512]
            style_net = generator.module.stylegan.synthesis.__getattr__('layer%d' % i).style
            w_s = style_net(w[:, layer_list[i]])
            w_t_s = style_net(w_t[:, layer_list[i]])
            w_t2_s = style_net(w_t2[:, layer_list[i]])
            Ws.append(w_s)
            W_ts.append(w_t_s)
            W_t2s.append(w_t2_s)
        ...
        for i in range(len(layer_list)):
            if i == 0:
                orthogonal = torch.mean(torch.pow(torch.mm(D_list[i], torch.transpose(D_list[i], 0, 1)) -
                                                  torch.eye(args.n_dirs).cuda(), 2.0))
            else:
                orthogonal = orthogonal + torch.mean(torch.pow(torch.mm(D_list[i], torch.transpose(D_list[i], 0, 1)) -
                                                               torch.eye(args.n_dirs).cuda(), 2.0))
        ...
        if (iter + 1) % args.report_image == 0:
            ...
            for dir_idx in range(args.n_dirs):
                ...
                for int_i in range(n_interps):
                    ...
                    for i in range(len(layer_list)):
                        d_i = D_list[i][dir_idx].view(1, dim_z, 1, 1)
                        ...
                        codezs_tmp[:, layer_list[i], :] = 0.0
                        codezs_tmp[:, layer_list[i], :] += codezs_move[:, layer_list[i], :]

                    x_cur = generator([torch.from_numpy(codezs_tmp).cuda()], which_block=args.layer, post_model=True).\
                        detach().cpu()
                ...
                if (dir_idx + 1) % args.n_dir_per_sheet == 0:
                    ...
                    image_path = os.path.join(out_dir, 'save_image_iter_%d_D_%d-%d.png' % (
                    iter, dir_idx - args.n_dir_per_sheet + 1, dir_idx))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_id', default='StyleGAN2DPTMultiZ', type=str,
    ...
    parser.add_argument('--which_layers', default='1-4', type=str, help='which layers to use. ')
