...
def main(args):
    ...
    for iter in range(args.total_iterations):
        ...
        if (iter + 1) % args.report_model == 0:
            ...
            save_path = os.path.join(out_dir, 'checkpoints', 'D_iter%d_which_layer%s_layer.pt' % (iter + 1, args.which_layer))
            save_dicts = dict()
            save_dicts['D'] = D
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_id', default='MaximumTraversingMask', type=str,
                        help='experiment prefix for easy debugging. ')
    # Parameters for Multi-Code GAN Inversion
    parser.add_argument('--inversion_type', default='StyleGAN_Layerwise-z',
    ...
    parser.add_argument('--wgt_neg', default=1.0, type=float, help='The learning rate of the optimizer.')
